...
const Escalations = () => {
  ...
  useEffect(() => {
    ...
    const filtered = escalations.filter((escalation) => {
      const customerMatch =
        ...
        escalation.customer === customerFilter;
      const projectMatch =
        projectFilter === "All Projects" ||
        escalation.project === projectFilter;
      const priorityMatch =
        priorityFilter === "All Priorities" ||
        escalation.priority === priorityFilter;
      const statusMatch =
        statusFilter === "All Statuses" || escalation.status === statusFilter;
    ...
    const criticalIssues = filtered.filter(
    ...
    const avgResolutionRate =
      totalEscalations > 0
        ? Math.round((resolvedCount / totalEscalations) * 100)
        : 0;
    setFilteredKPIs({
      totalEscalations,
      criticalIssues,
  ...
  const monthlyVolumeData = useMemo(() => {
    ...
    filteredEscalations.forEach((e) => {
      const month = e.date_raised
        ? new Date(e.date_raised).toLocaleString("default", {
            month: "short",
            year: "numeric",
          })
        : "Unknown";
    ...
    return Object.entries(counts)
      .map(([month, escalations]) => ({ month, escalations }))
      .sort((a, b) => {
        ...
        <Card className="p-4">
          <div className="text-center">
            <div className="text-2xl font-bold text-gray-600">
              {filteredKPIs.avgResolutionRate}%
            </div>
        ...
        <CardHeader>
          ...
          {filteredEscalations.length !== escalations.length && (
            <div className="text-sm text-gray-600 mt-2">
              Showing {filteredEscalations.length} of {escalations.length}{" "}
              escalations
              {customerFilter !== "All Customers" && (
      ...
      <EditEscalationDialog
        ...
        onUpdate={(updatedEscalation) => {
          if (updatedEscalation && updatedEscalation.id) {
            setEscalations((prev) =>
              prev.map((e) =>
                e.id === updatedEscalation.id ? updatedEscalation : e
              )
            );
          } else {
            getEscalations().then((result) => {
