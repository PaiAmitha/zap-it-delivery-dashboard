...
export function AppSidebar() {
  ...
  const renderNavItem = (item: typeof mainNavItems[0]) => {
    const navButton = (
      <SidebarMenuButton asChild className="w-full group transition-all duration-300 ease-in-out h-12">
        <NavLink 
          to={item.url} 
          end 
          className={({ isActive }) => {
            ...
            const heightClasses = "h-12 min-h-[3rem]";
  ...
  return (
    <TooltipProvider>
      <Sidebar 
        collapsible="icon"
        className={`bg-white border-r border-gray-200 transition-all duration-300 ease-in-out fixed left-0 top-20 h-[calc(100vh-5rem)] z-40 ${
          isActuallyMobile ? "shadow-xl" : "shadow-sm"
        }`}
        style={{
          width: collapsed && !isMobile ? '4rem' : '16rem',
          minWidth: collapsed && !isMobile ? '4rem' : '16rem',
          maxWidth: collapsed && !isMobile ? '4rem' : '16rem'
        }}
      >
        <SidebarHeader className="border-b border-gray-200 px-4 py-4 transition-all duration-300">
          <div className={`flex items-center transition-all duration-300 ${
            ...
            <Button
              ...
              className={`hover:bg-gray-100 rounded-lg transition-all duration-300 flex-shrink-0 border border-gray-200 p-2 ${
                collapsed && !isMobile 
